...
async function updateSpreadsheet(config, creds, entries) {
  ...
  // console.log(`Row Count: ${sheet.rowCount}\nColomun Count: ${sheet.colCount}`)
  // Get the cells from the sheet that have data in them
  const oldCells = await promisify(sheet.getCells)({
    'min-row': 1,
    'max-row': sheet.rowCount, // TODO: Currently does not work on brand bew sheets
    'min-col': 1,
    'max-col': sheet.colCount, // TODO: Currently does not work on brand bew sheets
    'return-empty': false
  }).catch((err) => {
    throw err;
  });
