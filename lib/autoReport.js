...
async function downloadSalesForceReport (config, browser, cookies, reportsPath) {
  ...
  await salesForce._client.send('Page.setDownloadBehavior', {
    behavior: 'allow',
    downloadPath: reportsPath + '/' + config.reportName
  });
...
// Export Report
  ...
  // Downloads report.
  console.log('Downloading report: ' + config.reportName);
  await salesForce.click('[value="Export"]');
};

async function updateSpreadsheet(config, creds, entries) {
  ...
  async function removeStaleData() {
    // Get the cells from the sheet that have data in them
    const oldCells = await promisify(sheet.getCells)({
      'min-row': 1,
      'max-row': sheet.rowCount, // TODO: Currently does not work on brand bew sheets or empty sheets
      'min-col': 1,
      'max-col': sheet.colCount, // TODO: Currently does not work on brand bew sheets or empty sheets
      'return-empty': false
    }).catch((err) => {
      throw err;
    });
    console.log('Removing stale data from ' + info.title)
  ...
  async function inputNewData() {
    ...
    // all content that is currently in the selected sheet.page
    ...
    console.log('Inputting new data into ' + info.title)
  ...
  // if (sheet.rowCount < entries.length && sheet.colCount < entries[0].length) {
  //   inputNewData();
  // } else {
  //   removeStaleData();
  //   inputNewData();
