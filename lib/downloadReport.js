...
// Export Report
  ...
  await fs.watch(reportsPath, (eventType, filename) => {
    if (fs.existsSync(reportsPath + '/' + filename.substring(0, filename.length - 11))) {
      //
      console.log(`Finished downloading` + filename.substring(0, filename.length - 11));
      browser.close();
      process.exit();
    }
  });
